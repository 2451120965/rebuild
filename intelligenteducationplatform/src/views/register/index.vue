        ...
        <el-col class="l-c-r" :xs="24" :sm="24" :md="9" :lg="9">
          ...
          <div class="login-form">
            ...
            <div class="hasaccount"  style="cursor:pointer" @click="returnlogin">
              已有账号?<span style="color: #3d73dd; margin-left: 4px"
                >返回登录</span
              >
            </div>
          </div>
...
<script>
import { Sendmessage, register } from "@/api/user.js";
import { _throttle } from "@/utils/auth.js"; //节流函数
import { phoneRules} from "./config/registerrules.js";
...
export default {
  ...
  registeruser: _throttle(function () {
    this.$refs.loginFormRef.validate((valid)=>{
      if(valid) {
      	if(this.LoginForm.password) {
      		register({ username: this.LoginForm.phone, password: this.LoginForm.password, sms: this.LoginForm.code}).then((res) => {
      	if (res.code === 200) {
      	  this.$router.push("/login")
      	} else {
					res.message ?this.$message.warning(res.message) : this.$message.warning("注册失败")
				}}).catch(() => {  })}
      }
    })
  }),
